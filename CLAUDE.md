...
## Como trabajar
- **Convenciones de idioma:** a partir de ahora, todo texto tecnico NUEVO
  se escribe en ingles — comentarios de codigo, mensajes de consola
  (console.log/warn/error), entradas nuevas en docs/DECISIONS.md, y
  mensajes de commit. No aplica en retroactivo: el espanol ya existente
  en el codigo, en docs/DECISIONS.md o en el historial de commits no se
  reescribe solo por tocar el archivo — unicamente el contenido que se
  añada o reemplace de aqui en adelante. No afecta a los mensajes de cara
  al usuario (Notice, UI), que siguen pasando por i18n (en.ts/es.ts) como
  siempre.
